---@mod dap-pwsh PowerShell extension for nvim-dap
...
local function load_dap()
  ...
  assert(ok, 'nvim-dap is required to use dap-pwsh')
  return dap
end
local temp_path = vim.fn.stdpath 'cache'

--- Register the python debug adapter
---@param adapter_pwsh_path string|nil Path to the pwsh interpreter. Path must be absolute or in $PATH and needs to have the debugpy package installed. Default is `python3`
---@param opts SetupOpts|nil See |dap-python.SetupOpts|
function M.setup(adapter_pwsh_path, opts)
  local dap = load_dap()
  adapter_pwsh_path = adapter_pwsh_path and vim.fn.expand(vim.fn.trim(adapter_pwsh_path)) or 'pwsh'
  opts = vim.tbl_extend('keep', opts or {}, default_setup_opts)
  opts.bundle_path = '~/.local/share/nvim/mason/packages/powershell-editor-services'
  dap.adapters.ps1 = function(cb, config)
    if config.request == 'attach' then
    else
      local command_fmt =
      [[& '%s/PowerShellEditorServices/Start-EditorServices.ps1' -BundledModulesPath '%s' -LogPath '%s/powershell_es_dap.log' -SessionDetailsPath '%s/powershell_es_dap.session.json' -FeatureFlags @() -AdditionalModules @() -HostName nvim -HostProfileId 0 -HostVersion 1.0.0 -LogLevel Normal]]
      local command = command_fmt:format(opts.bundle_path, opts.bundle_path, temp_path, temp_path)
      cb({
        type = 'server',
        port = "${port}",
        executable = {
          command = adapter_pwsh_path,
          args = {
            "-NoLogo", "-NoProfile",
            "-Command", command
          },
  ...
  if opts.include_configs then
    local configs = dap.configurations.ps1 or {}
    dap.configurations.ps1 = configs
...
--  assert(ft == 'pwsh', 'test_method of dap-pwsh only works for powershell files, not ' .. ft)
